...
)
{
    public string GenerateMapping(SourceProductionContext sourceProductionContext)
    {
        ...
        return $$"""
                 ...
                 {
                     public static {{targetNameSpace}}.{{TargetClassName}} MapTo{{TargetClassName}}(this {{sourceNameSpace}}.{{SourceClassName}} source) =>
                        {{instantiation}}
                 }
                 """;
    }
        ...
        if (
            ...
            return TargetType() switch
            {
                ClassDeclarationSyntax classDeclaration => classDeclaration
                    ...
                    .Select(targetParameter => new PropertyOrParameter(
                ...
                RecordDeclarationSyntax recordDeclaration => recordDeclaration
                    .ParameterList?.Parameters.Select(targetParameter => new PropertyOrParameter(
                        targetParameter.Type,
                        targetParameter.Identifier.Text
                    ))
    ...
    private IEnumerable<PropertyOrParameter>? GetSourceProperties() =>
        SourceType() switch
        {
            ClassDeclarationSyntax sourceClass => sourceClass
                .Members.OfType<PropertyDeclarationSyntax>()
                .Select(member =>
                    (Member: member, Attribute: member.GetMatchingMappingAttribute(TargetClassName))
                )
                .Select(type => new PropertyOrParameter(
                    type.Member.Type,
                    type.Member.Identifier.Text,
                    type.Attribute
                )),
    ...
    public string? TargetNameSpace()
    {
        return
            context.SemanticModel.GetDeclaredSymbol(targetType)
                is not INamedTypeSymbol sourceClassSymbol
                ? null
    ...
    private TypeDeclarationSyntax SourceType() =>
        mappingKind switch
        {
    ...
    private TypeDeclarationSyntax TargetType() =>
        mappingKind switch
        {
