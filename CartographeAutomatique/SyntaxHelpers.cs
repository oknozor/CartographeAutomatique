...
{
    public static AttributeSyntax? GetMatchingMappingAttribute(this MemberDeclarationSyntax memberDeclarationSyntax,
        string targetClassName)
    {
        ...
        return targetMappingCandidates.Count switch
        {
            ...
            _ => targetMappingCandidates.SingleOrDefault(x =>
                x.ArgumentList != null && x.ArgumentList.Arguments.Any(arg =>
    ...
    private static string ImplicitPrimitiveConversionTo(this ITypeSymbol source, ITypeSymbol target,
        string sourceIdentifier)
    {
        ...
        return (source.SpecialType, target.SpecialType) switch
        {
            ...
            (SpecialType.System_String, SpecialType.System_Int64) => $"Int64.Parse({sourceIdentifier})",
            (SpecialType.System_String, SpecialType.System_UInt16) => $"Int16.Parse({sourceIdentifier})",
            (SpecialType.System_String, SpecialType.System_UInt32) => $"UInt32.Parse({sourceIdentifier})",
            (SpecialType.System_String, SpecialType.System_UInt64) => $"UInt64.Parse({sourceIdentifier})",
    ...
    public static string? ImplicitConversionTo(this ITypeSymbol source, ITypeSymbol target, string sourceMemberAccess)
    {
        var targetType = Array1Symbol.FromTypeSymbol(target) ??
                         Collection1Symbol.FromTypeSymbol(target) ?? UnknownTypeSymbol.FromTypeSymbol(target);
        var sourceType = Array1Symbol.FromTypeSymbol(source) ??
                         Collection1Symbol.FromTypeSymbol(source) ?? UnknownTypeSymbol.FromTypeSymbol(source);

        if (targetType is null || sourceType is null) return null;
        if (targetType.SameAs(sourceType)) return sourceMemberAccess;
        ...
        var collectionImplicitConversion =
            sourceType.InnerTypeSymbol().ImplicitConversionTo(targetType.InnerTypeSymbol(), "t");
    ...
    public static bool IsCollection(this ITypeSymbol? targetSymbol) => targetSymbol?.ContainingNamespace.ToDisplayString() == "System.Collections.Generic";
